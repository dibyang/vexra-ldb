...
public class Compaction {
  ...
  public int getCfId() { return cfId; }

  public void setCfId(int cfId) { this.cfId = cfId; }
  ...
  public void addInputDeletions(VersionEdit edit) {
    for (FileMetaData input : levelInputs) {
      edit.deleteFile(cfId, level, input.getNumber());
    }
    for (FileMetaData input : levelUpInputs) {
      edit.deleteFile(cfId, level + 1, input.getNumber());
    }
  }
  ...
  public boolean isBaseLevelForKey(Slice userKey) {
    ...
    for (int level = this.level + 2; level < NUM_LEVELS; level++) {
      List<FileMetaData> files = inputVersion.getFiles(cfId, level);
