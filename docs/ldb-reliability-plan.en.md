...
- Windows directory-force compatibility: `AccessDeniedException` caused by Windows/JDK not supporting directory open through a file channel is still recorded in `ldb.fileSystemStats`, but the default log level is lowered to DEBUG so benchmark and release-gate logs are not flooded by known platform noise.
