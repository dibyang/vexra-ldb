...
- 不在本阶段引入 MergeOperator、PrefixExtractor、事务、TTL、二级索引或非空列族 drop/rename。
...
| 列族 | 支持打开前 `Options.addColumnFamily`，并支持运行时 `list/create/drop-empty` 最小生命周期 | 非空 drop、rename 和列族迁移 tombstone 仍不支持 |
...
| Checkpoint/Backup | checkpoint、全量备份和增量备份 Java API 已支持 | checkpoint 目标目录必须为空；增量备份当前发布完整可恢复目录 |
