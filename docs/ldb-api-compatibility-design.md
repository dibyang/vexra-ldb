...
下一版本面向 RocksDB 当前产品能力的差距拆分和工作包追踪见 [LDB 对齐 RocksDB 差距与下一版本规划设计](ldb-rocksdb-gap-next-version-plan.md)。
...
- 不在本阶段引入 MergeOperator、PrefixExtractor、事务、TTL、二级索引、custom Env 或完整 RocksDB CLI 兼容。
...
| 批量点查 | `get(List<byte[]>)` 和列族重载已支持 | 按输入顺序返回结果，未命中位置为 null；不改变磁盘格式 |
| 列族 | 支持打开前 `Options.addColumnFamily`，并支持运行时 `list/create/drop`、非空 drop tombstone 和 rename | dropped CF 物理 GC、列族迁移策略和大规模多列族运维仍是后续硬化项 |
...
| Checkpoint/Backup | checkpoint、全量备份、增量备份、对象仓库和清理 dry-run Java API 已支持 | checkpoint 目标目录必须为空；跨文件系统复制、低磁盘和长链备份仍需生产证据 |
| Group Commit | `Options.groupCommitEnabled` 可显式开启 | 默认关闭；WAL record 仍按请求分别编码 |
...
| `ldb.api.ecosystemGaps` | 关键未支持生态能力的阻塞原因 | 可新增字段 |
| `ldb.api.rocksdbGapPlan` | RocksDB 差距规划、下一版本目标和低风险实现项 | 可新增字段 |
| `ldb.recoveryEvidence` | 当前库目录的 WAL、MANIFEST、check/repair 入口和报告文件状态 | 可新增字段 |
| `ldb.backupEvidence` | checkpoint、backup、restore、对象仓库和清理 dry-run 的证据约定 | 可新增字段 |
| `ldb.columnFamilyEvidence` | 列族注册表、active/dropped 数量、MemTable、Level 文件和 lifecycle 策略摘要 | 可新增字段 |
| `ldb.prefixReadiness` | PrefixExtractor、prefix bloom 和 cache warmup 的启用前置条件与当前 cache/filter 状态 | 可新增字段；只观测不改变读路径 |
...
| Group commit switch | `Options.groupCommitEnabled` | supported，默认关闭 |
...
| `multi_get` | `LDB#get(List<byte[]>)` | supported，保持输入顺序 |
...
运行时列族生命周期新增根目录 `COLUMN-FAMILIES` 注册表，用于记录 runtime column family 的 id/name。该文件不改变 WAL、MANIFEST 或 SST 格式；backup、checkpoint、check 和 repair 会识别并携带该文件。
...
4. 运维或测试读取 `ldb.operationStats`、`ldb.compactionStats`、`ldb.walPolicy`、`ldb.groupCommitStats`、`ldb.snapshotCursorStats` 等统计入口。
...
当前仓库已有 `net.xdob.vexra.ldb.tool.LdbTool` 最小 CLI 入口，实现只读 `check`、`properties`、`check-backup`、`repair-plan` 和显式副作用 `repair`、`backup`、`incremental-backup`、`restore`、`checkpoint`。实例级 `compactRange` 暂不进入 CLI。因此 `ldbToolCommands` 为 partial，`rocksdbToolCommands` 仍保持 unsupported。
...
| `ldb repair-plan <db>` | `LDBFactory.planRepair` | 否 | 不创建写锁 | dry-run `RepairReport` JSON，不写报告文件 |
| `ldb repair <db>` | `LDBFactory.repair` | 是 | 独占写锁 | `REPAIR-REPORT.json`、隔离文件、重建 MANIFEST/CURRENT 信息 |
| `ldb checkpoint <db> <target>` | `LDB.checkpoint` | 正常打开源库、通过临时目录写目标目录 | 目标目录必须不存在或为空；成功后才发布 | `CHECKPOINT-REPORT.json`，包含复制/硬链接统计 |
...
| `ldb incremental-backup <db> <backup-root>` | `LDBFactory.createIncrementalBackup` | 读源库、写备份目录 | 离线校验源库 | `BackupReport` JSON 和校验结果 |
| `ldb check-backup <backup>` | `LDBFactory.checkBackup` | 否 | 不创建写锁 | `CheckReport` JSON |
| `ldb properties <db> [property...]` | `LDB.getProperty` | 否 | 默认只读打开 | property 键值输出 |
| `ldb scan <db> [limit]` | `LDB.newSnapshotCursor` | 否 | 默认只读打开 | 默认列族 key 顺序 JSON，key/value 使用 base64，默认最多 100 条 |
...
- `check`、`properties` 和 `scan` 必须默认只读，不创建新 WAL，不修改 MANIFEST。
- `repair`、`compact`、`restore` 属于写命令，必须在帮助和日志中明确有副作用；当前已开放的 `repair` 成功后直接输出 `REPAIR-REPORT.json`。
- `checkpoint` 和 `backup` 不应修改源库语义，但会写目标目录；目标目录非空应失败。`checkpoint` 通过临时目录构建，校验通过后发布，失败清理临时目录并保留异常原因。当前已开放的 `backup`/`restore` 直接复用离线备份报告，报告失败返回退出码 `2`；`checkpoint` 输出目标目录内的 `CHECKPOINT-REPORT.json`。
- 所有命令应支持机器可读输出，优先 JSON；面向人的文本输出只能作为附加视图。
- 当前 LDB 工具命令已有 `check`、`properties`、`repair`、`backup`、`restore`、`checkpoint`、`incremental-backup` 和 `check-backup` 最小入口；`ldb.api.unsupportedFeatures` 中的 `rocksdbToolCommands` 表示尚不兼容 RocksDB 原生命令集。

### 实现前置条件

1. 新增或扩展 CLI 命令时必须补命令解析测试，不复用测试里的临时 `main`。
2. 所有写命令必须覆盖锁冲突、路径不存在、目标目录非空和权限失败。
3. `properties` 命令必须覆盖 `ldb.api.*`、统计 property 和未知 property。
4. `scan` 命令必须覆盖默认 limit、显式 limit、坏 limit、只读不改库和 base64 输出。
5. 命令输出字段必须与 repair/check/backup 报告兼容，不能另起一套含义相近但字段不同的格式。
...
PrefixExtractor 若后续实现，必须先证明与当前 `DBComparator`、`FilterPolicy`、`SnapshotCursor`、range delete 和列族级 compaction 的组合语义。任何 prefix bloom 或 prefix seek 优化都必须有退化路径，不能让错误 prefix 配置导致漏读。下一版本 23.5 仅新增 `ldb.prefixReadiness` 观测属性，用来记录 `prefixExtractor=unsupported`、`prefixBloom=unsupported`、cache warmup 状态、当前 comparator/filter/cache 配置和启用前置条件；它不得改变 point get、MultiGet、cursor、range delete 或 compaction 行为。
...
| 23.1 | RocksDB 高级 API 兼容评审 | MergeOperator、PrefixExtractor、TTL、transactions 明确继续 unsupported 或选出一个低风险最小实现 |
