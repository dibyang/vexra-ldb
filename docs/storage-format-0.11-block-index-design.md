...
默认启用前的 writer 准入必须按生产风险而不是实验风险收紧。当前实现把单个 data block 的 local-index 预估空间放大上限从 `2000000ppm` 降到 `250000ppm`；超过阈值时不写该 block 的 local index，不声明 `block.local_index.v1`，并通过 `blockLocalIndexSkippedSpaceBlocks` 留下证据。该阈值仍不是最终 SLA，但它让默认化候选 SST 先避开明显高空间放大样本。

默认化还需要避免“无索引收益也升级格式”。因此 writer 现在按实际写出的 feature 计算 effective table format version：如果 `writeBlockLocalIndex=true` 但所有 data block 都被准入跳过，SST 仍保持调用方原始格式，默认情况下就是 legacy v1；只有真实写出 local-index directory 时才提升为 v3 并声明 `block.local_index.v1`。这为后续把 `writeBlockLocalIndex` 作为默认候选打开提供了兼容性缓冲。

基于上述缓冲，`Options.writeBlockLocalIndex()` 默认值已进入自动候选模式。默认 `tableFormatVersion` 仍是 1，但 writer 会在每个 SST 完成时按实际 feature 决定是否提升：无索引收益时继续写 legacy/v1，有通过准入的 local-index directory 时才写 v3。该策略把默认启用的影响面限制在“实际有索引证据”的 SST 上。
