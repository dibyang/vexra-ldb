...
- check/repair 报告新增 v3 block-local index 格式证据：`storageFormat` 和 `tableFormats` 现在归档 `v3Tables`、`blockLocalIndexTables`、索引字节数、覆盖 block 数以及每个 SST 的 block-local index policy/interval，作为后续 V3E 自检和 releaseGate 的基础。
- 新增 v3 文件格式演进设计文档，固化 v1/v2/v3 兼容矩阵、meta block registry、格式自检字段、block-local index 默认开启前置证据和 user-managed 发布门禁边界。
- 为 0.10.0 readrandom 70% 专项新增计划文档，并引入 SST direct point lookup 路径，减少随机点查和 MultiGet 对通用 iterator 链的依赖。
- 新增 MemTable 最新点值索引，在无 range tombstone 且 snapshot 覆盖最新版本时加速 warm readrandom；200k read_optimized warm_readrandom 达到 RocksDB JNI 的 98.52%。
- 归档 200k cold_readrandom 与 multiget_random 对比基线：read_optimized profile 分别达到 RocksDB JNI 的 64.52% 和 65.57%，作为后续 SST/block/cache 深层优化基线。
- MultiGet 接入 batch direct get：同一 SST 内按 data block handle 分组，同一 block 只打开一次；默认随机批量读仍逐 key 执行 block direct seek，避免稀疏 key 顺序扫描退化。
- 归档 batch block reuse 与 restart-key cache 最佳观测结果：200k read_optimized `multiget_random` 从 216,561.966 ops/s 提升到 245,912.746 ops/s，LDB 自身提升 13.55%，最佳观测达到 RocksDB JNI 的 116.02%；后续同机短跑复测存在明显波动，该数据作为 benchmark 证据而非稳定 SLA。
- 新增 `Options.blockCacheWarmOnOpen(true)` 随机读预热选项，`ldbDbBenchReport` 支持 `-Pldb.dbBench.blockCacheWarmOnOpen=true` 显式启用，并在 `ldb.sstReadStats` 暴露 `blockCacheWarmupTables` / `blockCacheWarmupBlocks`。
- 归档显式 block cache open-time warmup 与 restart-key cache 最佳观测结果：200k read_optimized `cold_readrandom` 从 184,805.871 ops/s 提升到 267,264.591 ops/s，LDB 自身提升 44.62%，最佳观测达到 RocksDB JNI 的 106.65%；后续同机短跑复测存在明显波动。
- `Block` 构造时缓存 restart point 完整 key，`Block.seek` / `Block.seekAll` 的 restart 二分不再重复解码 restart entry，为后续 block-local index 做内存侧验证。
- 评估但不启用 full-entry 内存块内 seek index：200k cold_readrandom 与稀疏随机 MultiGet 结果均显示回退；当前发布保留已验证正收益的 restart-key cache 和同 block open 复用，更紧凑的持久化 block-local index 留给后续文件格式版本。
- 评估但不启用同一 SST 内 index-block 批量定位：200k 稀疏随机 MultiGet 结果回退；当前发布保留 per-key index seek 加同 block data-block 复用。
- 新增 0.11.0 SST block-local index 文件格式设计，规划 opt-in v3 table format，用持久化紧凑块内索引替代 0.10.0 已验证回退的 full-entry 内存索引实验。
- 发布后版本升级为 0.10.0-SNAPSHOT；`publishUserManagedRelease` 增加上传后自动提交 open manual staging repository 的闭环任务，但不自动执行最终 Central release。
## [0.9.0] - 2026-06-20
...
### V3E-08 写入侧 local-index admission

- 在 `TableBuilder` 写入侧增加 local-index admission：按 `blockLocalIndexInterval` 计算后少于 2 个 anchor 的 data block 不再写 local-index block。
- properties 只在 SST 实际包含 local-index 目录项时声明 `block.local_index.v1` 与 `ldb.table.block_local_index=true`，避免小表/稀疏 interval 出现“声明了索引但无目录”的格式语义风险。
- 200k 样本：interval=1 + admission 下 same-block MultiGet 255,594.549 ops/s，目录加载 12 张表、5556 个目录项、190315 次 hit；cold 与 scan 不加载目录。interval=4 在默认布局下全部 admission 跳过，作为安全降级路径保留。
### V3E-09 冷随机读 block cache admission

- 新增 `Options.blockCacheAdmissionMinReads(int)`，默认值为 1，保持历史 miss 后立即缓存行为；设置为 2 时，direct get/MultiGet 的 data block 需要二次触达才进入 block cache。
- `BlockCache.stats()` 新增 `admissionRequests`、`admissionSkips`、`admissionAdmits`，用于观测 cold/random 路径是否触发 admission。
- 200k read-optimized 相邻样本中，admission=2 相比 admission=1：cold_readrandom 从 177,048.621 提升到 187,734.903 ops/s，multiget_random 从 161,744.394 提升到 178,359.627 ops/s，multiget_sameblock 从 216,031.923 提升到 274,764.640 ops/s；scan 不触发 admission。
### V3E-09 RocksDB JNI 随机读对照
